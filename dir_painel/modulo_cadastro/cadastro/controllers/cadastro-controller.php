...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $autoriza_uso_em_pesquisa = $_POST['autoriza_uso_em_pesquisa'];
    ...
    $sql = "INSERT INTO usuarios (nome_completo, psicologo, crp, data_nascimento, idade, cpf, profissao, email, celular, cep, endereco, numero, complemento, bairro, cidade, estado, estado_civil, naturalidade, escolaridade, senha, autoriza_uso_em_pesquisa) 
            VALUES (:nome_completo, :psicologo, :crp, :data_nascimento, :idade, :cpf, :profissao, :email, :celular, :cep, :endereco, :numero, :complemento, :bairro, :cidade, :estado, :estado_civil, :naturalidade, :escolaridade, :senha: autoriza_uso_em_pesquisa)";

    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':nome_completo', $nome_completo);
    $stmt->bindParam(':psicologo', $psicologo);
    ...
    $stmt->bindParam(':autoriza_uso_em_pesquisa', $autoriza_uso_em_pesquisa);
    ...
    if ($stmt->execute()) {
        ...
        try {
            ...
            $corpo_email .= "<a href='https://painel.psicologosespecialistas.com.br/confirmar_cadastro.php?email=" . urlencode($email) . "&token=" . md5($email . time()) . "'>Confirmar Cadastro</a><br><br>";
